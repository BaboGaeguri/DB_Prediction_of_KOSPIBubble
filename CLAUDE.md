...
여기서 $\tau$는 향후 관측 윈도우 크기(기본값: 3개월), $Y_t$는 시점 $t$의 PER이다. 한 시점이 bubble이면서 동시에 bubble_up 또는 bubble_down 중 하나로 분류되므로 multilabel 구조에 해당한다.

> **[구현 변경]** 원 Atsiwo(2025) 방법론의 $Y_t$는 P/D ratio이나, KOSPI 배당수익률은 주가와 배당금이 함께 움직여 PSY 폭발적 성장 감지 불가. PER(주가수익비율)로 대체 — 버블 시 이익 대비 주가가 빠르게 오르므로 PSY에 적합.
...
2. Monte Carlo 시뮬레이션으로 임계값 산출 (pipeline.py: 99%/95%/90% 계층적 선택, pipeline_test.py: 95% CV)
3. $BSADF_t > CV$ 이면 $bubble_t = 1$, 아니면 $bubble_t = 0$
4. $bubble_t = 1$인 시점에서 향후 $\tau$개월 평균 PER과 현재 PER을 비교하여 bubble_up / bubble_down 분류
5. 생성된 라벨(bubble, bubble_up, bubble_down)을 이후 분류 모델의 입력으로 사용
...
### PSY 검정 파라미터 설정 (r0_obs 고정값 채택)

PSY 권장 공식 $r_0 = 0.01 + 1.8/\sqrt{T}$는 T=252에서 $r_{0,obs} = 29$개월을 산출한다. 이는 KOSPI 중단기 버블 에피소드를 희석시켜 검출 불가 문제를 야기한다.

**채택값**: $r_0 = 15 / T$ → $r_{0,obs} = 15$개월 고정 (`pipeline_test.py` line 142)

**선택 근거**:
- 통계적 하한: ADF 신뢰성을 위해 최소 ~15개월 관측 필요
- 검출 상한: 목표 버블(중단기, 36~60개월)의 절반 이하로 설정 → 15 < 36 충족
- Mulenga & Ji(2022) 등 선행 연구에서 PSY 공식 대신 고정 $r_0$ 사용 사례 존재
...
```
D.Y.csv (2005/01~2025/12, 252개월)
  → pipeline_test.py → bubble_labels (내부, r0_obs=15개월 고정)
                              ↓
ALL DATA.csv (2005/01~2025/12, 252개월)  ─┐
뉴스심리지수(실험적 통계).csv (2005/01~2025/12) ─┤→ pipeline_test.py → model_results_test.csv
bubble 라벨 (내부 생성)                    ─┘
```
...
| 모형 | 설명 |
|------|------|
| **XGBoost (Tuned)** (주요 모형) | 그래디언트 부스팅 앙상블. RandomizedSearchCV(n_iter=12) + TimeSeriesSplit(n_splits=3)으로 max_depth / n_estimators / learning_rate / scale_pos_weight 최적화 |
...
## 3.2.6 테스트 실행 결과 (pipeline_test.py, 2026-02-25)

PSY 버블 미검출로 인해 BSADF 상위 20% 임계값(테스트용 폴백)을 적용한 결과:

- 데이터: D.Y.csv 2005/01~2025/12 (252개월), 학습 176개월 / 평가 76개월
- 버블 비율: 학습 21.0% / 평가 14.5% (테스트 폴백 기준, BSADF 상위 20%)
- XGBoost 최적 파라미터: max_depth=2, n_estimators=200, learning_rate=0.1, scale_pos_weight=6

| 모형 | bubble PR-AUC | bubble F1 | Balanced Acc | F1-macro |
|------|--------------|-----------|--------------|---------|
| **XGBoost (Tuned)** | 0.3995 | **0.4186** | **0.7322** | **0.3534** |
| Logistic Regression | **0.6161** | 0.2895 | 0.5846 | 0.2166 |
| Dummy (Always 0) | 0.1447 | 0.0000 | 0.5000 | 0.0000 |

XGBoost (Tuned) > Logistic Regression > Dummy 순서 (PR-AUC 제외).
LR은 PR-AUC 우세(확률 랭킹), XGBoost는 F1·BA·F1-macro 우세(임계값 분류).
**단, PSY n_sim=20 불안정으로 폴백 라벨 사용 중 → 정식 실행(PSY_test.py n_sim=500) 재확인 필요.**
...
| 파일 | 역할 | 상태 |
|------|------|------|
| `pipeline.py` | 정식 파이프라인 (PSY 내장 n_sim=500 + ML 튜닝) | 완성 |
| `pipeline_test.py` | 빠른 테스트 (PSY 내장 n_sim=20, 폴백 로직) | 완성 |

**실행 순서 (정식)**: `python pipeline.py` (약 15~20분 소요)
